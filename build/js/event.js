class ModelEvent {
  constructor() {
    this.events = [{
      ...
      id: 2,
      nameEvent: "meet",
      memberEvent: ["serhij", "marij", "petro", "ann", "oleg", "tamara"],
      dateEvent: "wed",
      timeEvent: "12"
    }];
...
class ViewEvent {
  constructor() {
    // The root element
    this.app = this.getElement('section', 'section_one_event'); // The cover element of the app

    this.wrapper = this.createElement('div', 'wrapper'); //The container for event form

    this.containerEvent = this.createElement('div', 'create_event_field'); //The container for event error

    this.eventError = this.createElement('div', 'error_event', 'hidden_error'); //The event error elements
    ...
    this.error.append(this.iconError, this.textError); //The event error btn

    this.errorBtn = this.createElement('span', 'error_event_btn');
    this.errorBtn.textContent = '&#10006;'; // The event error container

    this.eventError.append(this.error, this.errorBtn); // ----------------- The event body ----------------

    this.eventBody = this.createElement('div', 'event_body');
    this.eventField = this.createElement('div', 'added_field'); // ----------------- The event form ----------------

    this.eventForm = this.createElement('form');
    this.eventForm.setAttribute("action", "/calendar.html");
    this.eventForm.setAttribute("method", "GET"); // ----------------- The event form title----------------
    ...
    this.eventName.append(this.eventNameLabel, this.eventNameInput); // ----------------- The event form members----------------
    ...
    this.eventMembers.append(this.MemberLabel, this.MemberSelect); // ----------------- The event form day----------------
    ...
    this.eventDay.append(this.DayLabel, this.DaySelect); // ----------------- The event form time----------------
    ...
    this.eventForm.append(this.eventName, this.eventMembers, this.eventDay, this.eventTime); // ----------------- The event button----------------
...
event.model.checkEvent("lectures", "serhij", "fri", "12"); // let eventName = document.querySelector('#name');
